...
Remote control via long-poll bot (`telegram` subcommand). Only `TELEGRAM_CHAT_ID` may send commands. **Never logs private keys.**
...
### Commands
- `/doctor` `/status` `/help` `/panel_hint`
- `/snipe_public <nft> <qty> <at> [early_ms] [dry]`
- `/snipe_wl <slug> <qty> <at> [early_ms] [dry]`

Append `dry` to skip broadcast. Live snipes spend gas from `WALLET_KEY` on the host running the bot.
