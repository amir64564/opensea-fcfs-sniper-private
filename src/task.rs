...
impl TaskState {
    ...
    pub fn is_terminal(self) -> bool {
        matches!(self, Self::Idle | Self::Success | Self::Failed | Self::Cancelled)
    }
...
impl TaskGate {
    ...
    pub fn begin_waiting(&self) -> eyre::Result<()> {
        let cur = self.state();
        if cur.is_active() {
            eyre::bail!("duplicate execution blocked — task already {} (cancel first)", cur.as_str());
        }
        self.inner.cancel.store(false, Ordering::Release);
        self.inner.fired.store(false, Ordering::Release);
        self.inner.state.store(TaskState::Waiting as u8, Ordering::Release);
        crate::outln!("task → WAITING");
        Ok(())
    }
    pub fn set_armed(&self) {
        if self.is_cancelled() { return; }
        self.inner.state.store(TaskState::Armed as u8, Ordering::Release);
        crate::outln!("task → ARMED");
    }
    pub fn try_begin_firing(&self) -> bool {
        if self.is_cancelled() { return false; }
        if self.inner.fired.compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire).is_err() {
            crate::outln!("task double-fire blocked");
            return false;
        }
        self.inner.state.store(TaskState::Firing as u8, Ordering::Release);
        crate::outln!("task → FIRING");
        true
    }
    pub fn finish_success(&self) {
        self.inner.state.store(TaskState::Success as u8, Ordering::Release);
        crate::outln!("task → SUCCESS");
    }
    pub fn finish_failed(&self) {
        self.inner.state.store(TaskState::Failed as u8, Ordering::Release);
        crate::outln!("task → FAILED");
    }
    pub fn request_cancel(&self) {
        self.inner.cancel.store(true, Ordering::Release);
        self.inner.state.store(TaskState::Cancelled as u8, Ordering::Release);
        crate::outln!("task → CANCELLED");
    }
    pub fn reset_idle(&self) {
        self.inner.cancel.store(false, Ordering::Release);
        self.inner.fired.store(false, Ordering::Release);
        self.inner.state.store(TaskState::Idle as u8, Ordering::Release);
    }
}
...
pub fn clear_global_cancel() { global_cancel().store(false, Ordering::Release); }
pub fn request_global_cancel() { global_cancel().store(true, Ordering::Release); }
pub fn global_cancelled() -> bool { global_cancel().load(Ordering::Acquire) }
pub fn global_cancel_flag() -> &'static AtomicBool { global_cancel() }
